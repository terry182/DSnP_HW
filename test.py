...
class CmdReaderTester():
    ...
    def buffer(self, rtrim=False):
        ...
        if rtrim:
            pos = self.length
            # ori = self.buffer()
            # print('triming curr: [{}]'.format(PrintHelper.highlight(ori, self.cursor_position)))
            # print('triming pos : [{}]'.format(PrintHelper.highlight(ori, pos)))
            # truncate until either non-space or cursor
            while pos > 0 and self._buffer[pos - 1] == b' ' and self.cursor_position < (pos - 1):
                pos -= 1
            #     print('moving back [{}], branch {} and {}.'.format(
            #         PrintHelper.highlight(ori, pos),
            #         self._buffer[pos - 1] != b' ',
            #         self.cursor_position < (pos - 1)
            #     ))
            # print('trim over   : [{}], branch {} and {}.'.format(
            #     PrintHelper.highlight(ori, pos),
            #     self._buffer[pos - 1] != b' ',
            #     self.cursor_position < (pos - 1)
            # ))
...
def cross_testing():
    ...
    p2 = CmdReaderTester(['./cmdReader-ref'], 65536)
    ...
    while True:
        ...
        try:
            ...
            print('{:4}: O / SUCCESS - {}'.format(cnt, ''))
        except AssertionException as e:
            print('{:4}: X / FAILURE - {}'.format(cnt, ''))
            print(e)
            break
